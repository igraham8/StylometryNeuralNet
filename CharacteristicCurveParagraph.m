...
for ii = 1:length(ParaLH)
    ...
    SumH = [SumH ; sum(Hwords(First:Second,:),1)/sum(sum(Hwords(First:Second,:),1))];
end
...
for ii = 1:length(ParaLM)
    ...
    SumM = [SumM ; sum(Mwords(First:Second,:),1)/sum(sum(Mwords(First:Second,:),1))];
end
...
for ii = 1:length(ParaLHM)
    ...
    SumHM = [SumHM ; sum(HMwords(First:Second,:),1)/sum(sum(HMwords(First:Second,:),1))];
end
...
%{
Hwords = strsplit(Hstr);
Mwords = strsplit(Mstr);
HMwords = strsplit(HMstr);

Hcount = strlength(Hwords);
Mcount = strlength(Mwords);
HMcount = strlength(HMwords);

[HGC, HGR] = groupcounts(Hcount');
[MGC, MGR] = groupcounts(Mcount');
[HMGC, HMGR] = groupcounts(HMcount');
...
H2 = (ones([1 , length(Hwords)])'*1);
M2 = (ones([1 , length(Mwords)])' * -1);
AUTHORS = [H2 ; M2];
...
AUTH_est(AUTH_est<0) = -1;
Difference = abs(AUTH_est - AUTHORS');
...
subplot(3,1,2)
title('Analyzing Known Federalist Papers Using Characteristic Curves')
...
plot(1:17,SumH, 'b')
plot(1:17, SumM,'r')
title('Mendenhalls Characteristic Curves of Composition')
xlabel('Word Length - Hamilton (blue), Madison (red)')
ylabel('Frequency')

subplot(3,1,3)
hold on
title('Analyzing Contested Federalist Papers Using Chracteristic Curves')
yline(0)
xline(ParaLHM+0.5)
